...
def pred_val_fold(predictor, fold, spec_meta_info):
    ...
    for i, row in train_folds_df.iterrows():
        ...
        spec_meta = spec_meta_info[row.fname]
        probs = np.concatenate([probs, spec_meta], axis=0)
...
if __name__ == "__main__":
    spec_meta_info = load_spec_meta_info(use_corrections=False)
    ...
    for fold in config.folds:
        ...
        print("Val predict")
        pred_val_fold(predictor, fold, spec_meta_info)
