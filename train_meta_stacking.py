...
PARAMS = {
    'nn_module': ('FCNet', {
        'in_channels': len(config.classes) * len(EXPERIMENTS) + 1 + config.audio.n_mels,
...
def train_fold(save_dir, train_folds, val_folds, folds_data, spec_meta_info):
    train_dataset = StackingDataset(folds_data, train_folds,
                                    get_transforms(True),
                                    DATASET_SIZE,
                                    spec_meta_info=spec_meta_info)
...
if __name__ == "__main__":
    ...
    spec_meta_info = load_spec_meta_info(use_corrections=CORRECTIONS)
    ...
    for fold in config.folds:
        ...
        train_fold(save_fold_dir, train_folds, val_folds, folds_data, spec_meta_info)
